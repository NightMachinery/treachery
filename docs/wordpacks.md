...
Use `treachery-assets cache <asset-pack-name>` to prewarm the cache. While it runs, the command prints progress to stderr: the selected asset pack, the asset directory being scanned, each registered source image, discovered cacheable image count, each source/cache path currently being ensured, and each completed cache file. This is useful because AVIF encoding can take a while on large packs such as `gouache-treachery`. Afterward it prints a verbose completion report with the selected CrimePack, asset pack, cache directory, geometry, total image count, and every source image's cache path and content ID. Use `treachery-assets migrate <asset-pack-name> [--no-clean-git-history]` to replace a pack's committed assets with AVIF files after creating a mirror backup in `~/tmp/backups/`; the migration command prints the same verbose image report after the migration completes.
